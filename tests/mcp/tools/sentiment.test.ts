...
describe('handleGetSentimentAnalysis', () => {
	it('returns aggregated sentiment data', async () => {
		const result = await handleGetSentimentAnalysis(mockDb as any, 'AcmeCorp', 30, 'tenant-1')
...
describe('handleGetCompetitiveIntel', () => {
	it('returns competitor sentiment comparison', async () => {
		const result = await handleGetCompetitiveIntel(mockDb as any, 'CompetitorCo', 'tenant-1')
